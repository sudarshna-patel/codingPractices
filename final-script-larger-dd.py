...
# distance distribution in a graph
paths = [nx.single_source_dijkstra_path_length(GC, n) for n in list(GC.nodes())[:1000]]
print('=================DONE calculating all pairs shortest path lengths')

l = [list(i.values()) for i in paths]
flat_list = np.array([j for sub in l for j in sub])
#print(flat_list)
